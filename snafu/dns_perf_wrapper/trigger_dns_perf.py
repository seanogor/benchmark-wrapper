...
class Trigger_dns_perf():
    ...
    def run_dns_perf_test(self):
        """ Runs dns-perf workload and returns the stdout """

        command = "dnsperf -l {} -s {} -Q {} -d {} -c {}".format(self.run_time, self.server_address, self.queries_per_second, self.data_file, self.clients)
        logger.info(command)
        try:
            ...
            logger.error("Failed to run %s, error: %s" % (command, e))
            exit(1)
        return output
...
[Status] Stopping after {{ stop_time_duration }} seconds
[Status] Testing complete (time limit)

Statistics:

  Queries sent:         {{ queries_sent }}
  Queries completed:    {{ queries_completed }} ({{ queries_completed_percentage }}%)
  Queries lost:         {{ queries_lost }} ({{ queries_lost_percentage }})

  Response codes:       {{ response_code }}
  Average packet size:  request {{ avg_request_packet_size }}, response {{ avg_response_packet_size }}
  Run time (s):         {{ runtime }}
  Queries per second:   {{ QPS }}

  Average Latency (s):  {{ avg_latency }} (min {{ latency_min }}, max {{ latency_max }})
  Latency StdDev (s):   {{ latency_stddev }}
        """
